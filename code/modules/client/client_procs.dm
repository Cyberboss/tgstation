...
STRING(image_bug_blacklist_message, "bug preventing client display overrides from working leads to clients being able to see things/mobs they shouldn't be able to see")

GLOBAL_LIST_INIT(blacklisted_builds, list(
	"1407" = _(image_bug_blacklist_message),
	"1408" = _(image_bug_blacklist_message),
...
/client/New(TopicData)
	...
	if (num2text(byond_build) in GLOB.blacklisted_builds)
		...
		to_chat(src, "<span class='danger'>Byond build [byond_build] ([byond_version].[byond_build]) has been blacklisted for the following reason:</span>")
		to_chat(src, GLOB.blacklisted_builds[num2text(byond_build)])
