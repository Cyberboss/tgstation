...
/datum/subsystem/shuttle/proc/requestTransfer(mob/user)
	requestEvac(user, "TRANSFER", 1)

/datum/subsystem/shuttle/proc/requestEvac(mob/user, call_reason, transfer_override = 0)
	//transfer_override for crew tranfer and or admin stuff
	//if 0 normal emergency
	//if 1 crew transfer
	//if 2 admin crew transfer
	if(!emergency)
	...
	if(world.time - round_start_time < config.shuttle_refuel_delay && transfer_override != 2)
		if(!transfer_override)
			...
			user << "The transfer shuttle is refueling. Please wait another [abs(round(((world.time - round_start_time) - config.shuttle_refuel_delay)/600))] minutes before trying again."
		return
	...
	if(!transfer_override)
		...
		switch(emergency.mode)
			if(SHUTTLE_RECALL)
				user << "The transfer shuttle may not be called while returning to Centcom."
				return
			if(SHUTTLE_CALL)
				user << "The transfer shuttle is already on its way."
				return
			if(SHUTTLE_DOCKED)
				user << "The transfer shuttle is already here."
				return
			if(SHUTTLE_IGNITING)
				user << "The transfer shuttle is firing its engines to leave."
				return
			if(SHUTTLE_ESCAPE)
				user << "The transfer shuttle is moving away to a safe distance."
				return
			if(SHUTTLE_STRANDED)
				user << "The transfer shuttle has been disabled by Centcom."
				return
