var/list/insulated_clothing = list(	/obj/item/clothing/head/helmet/space/hardsuit/engine,
...
/obj/machinery/power/arc_emitter/proc/fire_bolt()
	...
	*/
	...
	for(var/obj/machinery/power/supermatter_shard/supermatter/S in oview(src, 5))
		targets += S
	if(targets.len > 0)
		for(var/i=0, i <= 10, i+=5)
			var/obj/machinery/power/supermatter_shard/supermatter/S = pick(targets)
			spawn(rand(0, 10))
				arc(S)
				s_energize(S)
		return
	...
	for(var/obj/machinery/field/generator/G in oview(src, 5))
		targets += G
	if(targets.len > 0)
		for(var/i=0, i <= 10, i+=5)
			var/obj/machinery/field/generator/G = pick(targets)
			spawn(rand(0, 10))
				arc(G)
				G.arc_act(arc_power)
		return
...
/obj/machinery/power/arc_emitter/proc/c_energize(obj/machinery/power/fusion/core/c)
	c.heat += (arc_power*c.beam_coef*5)	//times 5 because this ticks SO MUCH SLOWER !
*/

/obj/machinery/power/arc_emitter/proc/s_energize(obj/machinery/power/supermatter_shard/supermatter/s)
	s.arc_act(arc_power)
...
/obj/machinery/power/arc_emitter/proc/insulated(var/mob/living/carbon/human/m)
	...
	if(isnull(m.head) || isnull(m.wear_suit))
		return 0
	for(var/h in insulated_clothing)
		if(istype(m.head, h))
			head = 1
	for(var/b in insulated_clothing)
		if(istype(m.wear_suit, b))
			body = 1
	if(body == 1 && head == 1)
		return 1
